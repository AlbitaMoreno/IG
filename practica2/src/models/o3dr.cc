...
void O3DR::Revolucionar(vector<_vertex3f> perfil, int nlados)
{
    ...
    float alpha_aux = 2 * M_PI / (float)nlados;
    ...
    for (int i = 0; i < nlados; i++)
    {
        for (int j = 0; j < perfil.size() - 1; j++)
        {
            ...
            triangulos.push_back(_vertex3i(punto, (punto + perfil.size() + 1) % max, (punto + perfil.size()) % max));
            triangulos.push_back(_vertex3i(punto, punto + 1, (punto + perfil.size() + 1) % max));
        }
    }
    ...
    if (tapa_abajo)
        for (int i = 0; i < nlados; i++)
        {
            triangulos.push_back(_vertex3i((i + 1) * (perfil.size()) - 1, vertices.size() - 1, (((i + 1) * (perfil.size()) - 1) + perfil.size()) % max)); // Triangulos de la cara de abajo.
        }

    if (tapa_abajo && tapa_arriba)
        for (int i = 0; i < nlados; i++)
        {
            triangulos.push_back(_vertex3i(vertices.size() - 2, i * perfil.size(), (((i + 1) * (perfil.size())) % max))); // Triangulos de la cara de arriba cuando tambien está presente la de abajo.
        }

    if (!tapa_abajo && tapa_arriba)
        for (int i = 0; i < nlados; i++)
        {
            triangulos.push_back(_vertex3i(vertices.size() - 1, i * perfil.size(), (((i + 1) * (perfil.size())) % max))); // Triangulos de la cara de arriba cuando no existe la de abajo.
        }
}
